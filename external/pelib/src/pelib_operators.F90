...
subroutine get_magnetic_gradient(fckmats)
    ...
    if (pelib_fd) then
        call lao_pde_nuc(fckmats)
        if (myid == master)  then
            call lao_pde_el_rep(fckmats)
        end if
    end if
...
!------------------------------------------------------------------------------
#ifdef VAR_PDE
subroutine lao_pde_nuc(fckmats)
    ...
    ! fd data
    real(rp), dimension(:,:), allocatable :: Rs_fd, Mks_fd
...
#if defined(VAR_MPI)
    call mpi_bcast(fdnucs, 1, impi, master, comm, ierr)
#endif

    allocate(Rs_fd(3,fdnucs), Mks_fd(1,fdnucs))

    if (myid == master) then
        call h5dopen_f(file_id, 'nuclear coordinates', dset_id, error)
        dim_2d = [3, fdnucs]
        call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, Rs_fd, dim_2d, error)
        call h5dclose_f(dset_id, error)

        call h5dopen_f(file_id, 'nuclear charges', dset_id, error)
        dim_2d = [1, fdnucs]
        call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, Mks_fd, dim_2d, error)
        call h5dclose_f(dset_id, error)
        call h5fclose_f(file_id, error)
        call h5close_f(error)
    end if
...
#if defined(VAR_MPI)
    call mpi_bcast(Rs_fd, size(Rs_fd), rmpi, master, comm, ierr)
    call mpi_bcast(Mks_fd, size(Mks_fd), rmpi, master, comm, ierr)
#endif
    ...
    if (nprocs == 1) then
        ...
        start = myid * (fdnucs / nprocs) + 1
        finish = (myid + 1) * (fdnucs / nprocs)
        if (myid == nprocs -1) finish = fdnucs
#endif
    end if

    Mk_ints = 0.0
    do site = start, finish
        if (maxval(abs(Mks_fd(:,site))) < zero) cycle
        call Mk_integrals('magnetic_gradient', Mk_ints, Rs_fd(:,site), Mks_fd(:,site))
    end do
...
end subroutine lao_pde_nuc

subroutine lao_pde_el_rep(fckmats)
    ...
    real(rp), allocatable, dimension(:) :: magnetic_derivative
    
    allocate(magnetic_derivative(size(fckmats)))
    call h5open_f(error)
    call h5fopen_f(trim(h5pdefile), H5F_ACC_RDONLY_F, file_id, error)
    call h5dopen_f(file_id, 'london electrostatic matrix', dset_id, error)
    if (error /= 0) then
        error stop 'Error: Electrostatic operator London contributions missing on hdf5 file'
    end if
    dim_1d = size(fckmats)
    call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, magnetic_derivative, dim_1d, error)
    call h5dclose_f(dset_id, error)
    fckmats = fckmats + magnetic_derivative
    call h5dopen_f(file_id, 'london exchange-repulsion matrix', dset_id, error)
    if (error /= 0) then
        error stop 'Error: Exchange-repulsion operator London contributions missing on hdf5 file'
    end if
    call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, magnetic_derivative, dim_1d, error)
    call h5dclose_f(dset_id, error)
    fckmats = fckmats + magnetic_derivative
